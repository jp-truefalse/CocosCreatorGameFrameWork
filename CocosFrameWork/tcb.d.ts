...
declare namespace cc {
  namespace cloud {
    interface ITencentCloudConfig {
      platform: 'tencent';
      tencent: cloudbase.ICloudbaseConfig;
    }

    /**
     * 初始化Cloudbase。
     *
     * @param config 【可选】初始化配置 ，如果没有传该参数，那么配置从assets/cloud/tcbjs/cloudConfig.js中读取
     * @param config.platform 平台，目前仅支持'tencent'
     * @param config.tencent 传给cloudbase.init的初始化参数，当平台为'tencent'时使用
     * @return {!cloudbase.app.App} 初始化成功的腾讯Cloudbase实例
     */
    function initialize(config?: ITencentCloudConfig):cloudbase.app.App;
    /*
      之后通过重载来区分多种云SDK的配置，比如如果添加了阿里云的支持，那么
      添加一个新的重载函数：
      function initialize(config?: IAliyunCloudConfig):阿里云实例类型;
      其中，IAliyunCloudConfig有一个platform类型为固定字符串'aliyun'，
      这样，大部分编辑器可以根据platform的值区分是哪种配置类型，给出正确的补全提示。
    */
  }
}
